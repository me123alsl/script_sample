...
create_ansible_hosts() {
    if [ $READ_SERVER_TYPE = "COMMAND" ]; then
        echo "$READ_SERVER_TYPE=COMMAND"
        echo "$MANAGED_SERVER=MANAGED_SERVER"
        sudo echo "" > $ANSIBLE_HOSTS
...
{
    copy-ssh() {
        echo -n $ANSIBLE_PASS | su $ANSIBLE_USER -c "ssh-keygen -b 2048 -t rsa -f ~/.ssh/id_rsa -q -N ''" 
        while read host; do
            echo "
            echo -n $ANSIBLE_PASS | su - $ANSIBLE_USER -c 
            sshpass -f $ANSIBLE_PASS ssh-copy-id -o StrictHostKeyChecking=no $ANSIBLE_USER@$host
            "
